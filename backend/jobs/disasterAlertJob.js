...
const {
	...
	getCountryCounts,
} = require("../utils/countryExtractor");
...
const initializeProcessedDisasters = async () => {
	try {
		...
		console.log(
			`Initialized with ${lastProcessedDisasterIds.size} existing disasters`
		);
	} catch (error) {
		console.error(
			"Failed to initialize processed disasters:",
			error.message
		);
	}
};
...
const formatAlertSubject = (disaster, country) => {
	...
	return `${emoji} ${disaster.severity.toUpperCase()} ${
...
const processNewDisasters = async () => {
	try {
       ...
       ORDER BY occurred_at DESC`
		...
		const newDisasters = disasters.filter(
			(d) => !lastProcessedDisasterIds.has(d.id)
		);
		...
		for (const [country, countryDisasters] of Object.entries(
			disastersByCountry
		)) {
			console.log(
				`Sending ${countryDisasters.length} alerts for ${country}...`
			);
			...
			for (const disaster of countryDisasters) {
				try {
					...
					console.log(
						`Alert sent for disaster #${disaster.id} in ${country}`
					);
					...
					console.error(
						`Failed to send alert for disaster #${disaster.id}:`,
						error.message
					);
				}
			}
...
const updateSnsTopics = async () => {
	try {
		...
		console.log(
			`Found disasters in ${Object.keys(countryCounts).length} countries`
		);
		...
		for (const [country, count] of Object.entries(countryCounts)) {
			...
			try {
				...
				console.error(
					`Failed to update topic for ${country}:`,
					error.message
				);
			}
		}
		...
		const activeCountries = Object.keys(countryCounts).filter(
			(c) => c !== "Unknown"
		);
		if (activeCountries.length > 0) {
         ...
         WHERE country NOT IN (${activeCountries
				.map((_, i) => `$${i + 1}`)
				.join(",")})`,
				activeCountries
			);
...
const startDisasterAlertJob = async () => {
	...
	console.log(
		`Next run: ${new Date(Date.now() + 10 * 60 * 1000).toLocaleString()}`
	);
