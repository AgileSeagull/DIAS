...
const { transformFire, filterValidDisasters, removeDuplicates } = require('../utils/dataTransformer');
const { reverseGeocode } = require('./geocodingService');

// Using Copernicus Emergency Management Service - public and free
const COPERNICUS_URL = 'https://emergency.copernicus.eu/mapping/list-of-components/EMSR';
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (fire.latitude && fire.longitude) {
        fires.push({
          ...
          brightness: parseFloat(fire.brightness || fire.bright_ti4 || '300'),
          frp: parseFloat(fire.frp || '0'),
          confidence: parseInt(fire.confidence || '85'),
          location_name: fire.country || fire.state || 'Unknown Location',
          title: `Wildfire detected in ${fire.country || fire.state || 'Unknown Location'}`,
          description: `Brightness: ${fire.brightness || fire.bright_ti4}K, Confidence: ${fire.confidence}%`,
...
const fetchFireData = async () => {
  try {
    ...
    // Transform fire data
    let transformedFires = fires.map((event, index) => {
      // Create stable ID based on location and date
      const idBase = `${event.latitude.toFixed(2)}-${event.longitude.toFixed(2)}-${new Date(event.acq_date).toISOString().split('T')[0]}`;
        ...
        if (existing.rows.length > 0) {
          await query(
            `UPDATE disasters 
             ...
             WHERE disaster_id = $1`,
            [
              fire.disaster_id, fire.severity, fire.title,
              fire.description, fire.location_name, fire.latitude,
              fire.longitude
            ]
          );
          updatedCount++;
          ...
          await query(
            ...
            [
              fire.disaster_id, fire.type, fire.severity, fire.title,
              fire.description, fire.location_name, fire.latitude,
              fire.longitude, fire.source, fire.occurred_at
            ]
          );
          newCount++;
        }
      } catch (err) {
        console.error(`❌ Failed to save fire ${fire.disaster_id}:`, err.message);
      }
    }

    console.log(`✅ Fires processed: ${newCount} new, ${updatedCount} updated`);
