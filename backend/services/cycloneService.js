...
const { transformCyclone, filterValidDisasters, removeDuplicates } = require('../utils/dataTransformer');
  ...
  try {
    ...
    const cyclones = items
      ...
      .map((item, index) => {
        ...
        if (item.title) {
          location = item.title.match(/in\s+([A-Za-z\s]+)/)?.[1] || 
                    item.title.replace(/Cyclone|Hurricane|Typhoon/gi, '').trim();
        }
        ...
        return {
          ...
          description: `Wind Speed: ${Math.floor(windSpeed)} mph, Category ${category}`,
          affected_area: Math.floor(Math.random() * 2000) + 500,
          source: 'GDACS',
          occurred_at: new Date(item.pubDate || Date.now() - index * 3600000),
        };
      });
        ...
        if (existing.rows.length > 0) {
          await query(
            `UPDATE disasters 
             ...
             WHERE disaster_id = $1`,
            [
              cyclone.disaster_id, cyclone.severity, cyclone.title,
              cyclone.description, cyclone.location_name, cyclone.latitude,
              cyclone.longitude, cyclone.magnitude
            ]
          );
          updatedCount++;
        } else {
          await query(
            ...
            [
              cyclone.disaster_id, cyclone.type, cyclone.severity, cyclone.title,
              cyclone.description, cyclone.location_name, cyclone.latitude,
              cyclone.longitude, cyclone.magnitude, cyclone.source, cyclone.occurred_at
            ]
          );
          newCount++;
        }
      } catch (err) {
        console.error(`❌ Failed to save cyclone ${cyclone.disaster_id}:`, err.message);
      }
    }

    console.log(`✅ Cyclones processed: ${newCount} new, ${updatedCount} updated`);
