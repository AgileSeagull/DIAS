...
const fetchAllDisasterData = async () => {
	...
	const [earthquakeResult, floodResult, fireResult, cycloneResult] =
	...
	if (earthquakeResult.status === "fulfilled") {
		...
		results.sources.earthquake = {
			success: false,
			error: earthquakeResult.reason.message,
		};
	}
	...
	if (floodResult.status === "fulfilled") {
		...
		results.sources.flood = {
			success: false,
			error: floodResult.reason.message,
		};
	}
	...
	if (fireResult.status === "fulfilled") {
		...
		results.sources.fire = {
			success: false,
			error: fireResult.reason.message,
		};
	}
	...
	if (cycloneResult.status === "fulfilled") {
		...
		results.sources.cyclone = {
			success: false,
			error: cycloneResult.reason.message,
		};
	}
...
const getAllDisasterStats = async () => {
	const [earthquakeStats, floodStats, fireStats, cycloneStats] =
		await Promise.all([
