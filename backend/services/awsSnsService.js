...
const createTopicForCountry = async (country) => {
	try {
		const topicName = `dias-alerts-${country
			.toLowerCase()
			.replace(/\s+/g, "-")}`;
...
const getOrCreateTopicForCountry = async (country) => {
	try {
		...
		console.error(
			`Failed to get/create topic for ${country}:`,
			error.message
		);
		throw error;
	}
...
const subscribeEmailToCountry = async (email, country, userId = null) => {
	try {
		...
		console.log(
			`Subscribed ${email} to ${country} alerts: ${subscriptionArn}`
		);
       ...
       DO UPDATE SET subscription_arn = $4, status = $5, subscribed_at = CURRENT_TIMESTAMP`,
		...
		console.error(
			`Failed to subscribe ${email} to ${country}:`,
			error.message
		);
		throw error;
	}
...
const publishAlert = async (country, subject, message, disasterId) => {
	try {
		...
		if (result.rows.length === 0) {
			console.log(
				`⚠️  No SNS topic found for ${country}, creating one...`
			);
			const topicArn = await createTopicForCountry(country);
			return await publishToTopic(
				topicArn,
				subject,
				message,
				country,
...
const publishWelcomeEmail = async (topicArn, subject, message) => {
	try {
		console.log(
			`⚠️ Welcome email attempt - Note: Will only send to CONFIRMED subscribers`
		);
		...
		console.log(
			`⚠️ User will receive this email AFTER they confirm subscription via AWS email`
		);
...
const updateDisasterCount = async (country, count) => {
	try {
		...
		console.error(
			`Failed to update disaster count for ${country}:`,
			error.message
		);
	}
};
...
const unsubscribeEmail = async (subscriptionArn) => {
	try {
		...
		console.error(
			`Failed to unsubscribe ${subscriptionArn}:`,
			error.message
		);
		throw error;
	}
