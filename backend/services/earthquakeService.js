...
const { transformEarthquake, filterValidDisasters, removeDuplicates } = require('../utils/dataTransformer');
...
const fetchEarthquakeData = async () => {
  ...
  while (retries > 0) {
    try {
      ...
      console.log(`📊 Found ${earthquakes.length} earthquakes (last 24 hours)`);

      // Filter by magnitude
      const filteredQuakes = earthquakes.filter(
        event => event.properties.mag >= MIN_MAGNITUDE
      );
      console.log(`✅ ${filteredQuakes.length} earthquakes meet magnitude >= ${MIN_MAGNITUDE}`);
      ...
      console.log(`💾 Processing ${validQuakes.length} valid earthquakes...`);
      ...
      for (const quake of validQuakes) {
        try {
          ...
          if (existing.rows.length > 0) {
            ...
            await query(
              `UPDATE disasters 
               ...
               WHERE disaster_id = $1`,
              [
                quake.disaster_id, quake.type, quake.severity, quake.title,
                quake.description, quake.location_name, quake.latitude,
                quake.longitude, quake.magnitude, quake.depth, quake.source,
                quake.external_url, quake.occurred_at
              ]
            );
            updatedCount++;
          } else {
          ...
          console.error(`❌ Failed to save earthquake ${quake.disaster_id}:`, err.message);
        }
      }

      console.log(`✅ Earthquakes processed: ${newCount} new, ${updatedCount} updated`);
      ...
      if (retries > 0) {
        console.warn(`⚠️  API call failed, retrying... (${retries} retries left)`);
        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY));
      }
    }
  }

  console.error('❌ Failed to fetch earthquake data after retries:', lastError.message);
  return {
    success: false,
    type: 'earthquake',
