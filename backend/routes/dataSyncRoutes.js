...
const { fetchAllDisasterData, fetchDisasterDataByType, getAllDisasterStats } = require('../services/disasterDataFetcher');
  ...
  try {
    ...
    if (!['earthquake', 'flood', 'fire', 'cyclone'].includes(type)) {
      return res.status(400).json({
        success: false,
        message: 'Invalid disaster type. Must be: earthquake, flood, fire, or cyclone',
      });
    }
