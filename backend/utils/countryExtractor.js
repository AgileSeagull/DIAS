...
const extractCountry = (locationName) => {
	...
	for (let i = parts.length - 1; i >= 0; i--) {
		...
		const partialMatch = KNOWN_COUNTRIES.find(
			(country) =>
				country.toLowerCase().includes(lowerPart) ||
...
const getCountryFromCoordinates = async (latitude, longitude) => {
	try {
		...
		if (response.data && response.data.address) {
			...
			if (country) {
				console.log(
					`Geocoded (${latitude}, ${longitude}) → ${country}`
				);
				return country;
			}
		}
...
const extractCountryFromDisaster = async (disaster) => {
	...
	if (disaster.latitude && disaster.longitude) {
		console.log(
			`⚠️  No country in location "${disaster.location_name}", trying coordinates...`
		);
	...
	// Fallback
	console.log(
		`⚠️  Could not determine country for disaster at (${disaster.latitude}, ${disaster.longitude})`
	);
	return "Unknown";
};
